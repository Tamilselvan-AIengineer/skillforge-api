...
def root():
    return {
        ...
        "version": "1.0.0",
        "endpoints": [
            "/ask", "/skill-gap", "/update-skill",
            "/projects/{goal}", "/roadmap/{goal}",
            "/interview-prep/{goal}"
        ]
    }
...
async def get_skill_gap(request: SkillGapRequest):
    """Returns missing skills and readiness % for a target role."""
    try:
        missing = get_missing_skills(request.student_skills, request.goal)
...
async def update_skill(request: UpdateSkillRequest):
    """Continual learning — add a newly learned skill."""
    try:
        new_doc = Document(
            page_content=(
                f"Student has learned: {request.new_skill}. "
                f"This is now part of their skillset for {request.goal}."
            ),
...
"""
        ...
        return {
            "message": f"✅ '{request.new_skill}' added!",
...
async def get_projects(goal: str, student_skills: str = ""):
    """Returns recommended projects based on missing skills."""
    try:
...
Suggest exactly 3 projects. For each:
- Project name
- Skills it teaches
...
async def get_roadmap(goal: str, student_skills: str = ""):
    """Returns a personalized learning roadmap."""
    try:
        skills_list = student_skills.split(",") if student_skills else []
        missing = get_missing_skills(skills_list, goal)
...
Each month: what to learn, free resources, and a mini project.
...
async def interview_prep(goal: str):
    """Returns interview preparation tips."""
    try:
...
Give interview preparation including:
- 5 common technical questions with brief answers
- 3 behavioral questions with STAR method tips
